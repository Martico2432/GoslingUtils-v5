...
class GoslingAgent(Bot):
    ...
    def get_ready(self, packet: flat.GamePacket):
        field_info = self.field_info
        for i in range(len(field_info.boost_pads)):
    ...
    def refresh_player_lists(self, packet: flat.GamePacket):
        # makes new freind/foe lists
        # Useful to keep separate from get_ready because humans can join/leave a match
        self.friends = [car_object(i, packet) for i in range(len(packet.players)) if
                        packet.players[i].team == self.team and i != self.index]
        self.foes = [car_object(i, packet) for i in range(len(packet.players)) if packet.players[i].team != self.team]
    ...
    def line(self, start, end, color=None):
        ...
        line_start = flat.Vector3(x=start.x, y=start.y, z=start.z)
        line_end = flat.Vector3(x=end.x, y=end.y, z=end.z)
        self.renderer.draw_line_3d(line_start, line_end, self.renderer.create_color(*color, 255))
    ...
    def debug_stack(self):
        ...
        for i in range(len(self.stack) - 1, -1, -1):
            ...
            self.renderer.draw_string_2d(text, 0.05, 0.05 + (0.05 * (len(self.stack) - i)), 1, white)

    def clear(self):
        # Shorthand for clearing the stack of all routines
        self.stack = []

    def preprocess(self, packet: flat.GamePacket):
        # Calling the update functions for all of the objects
        if len(packet.players) != len(self.friends) + len(self.foes) + 1: self.refresh_player_lists(packet)
        ...
        # if self.kickoff_flag == False and packet.match_info.match_phase == flat.MatchPhase.Active and packet.game_info.is_kickoff_pause:
        if self.kickoff_flag == False and packet.match_info.match_phase == flat.MatchPhase.Kickoff: # I think Active and kikcoff will never be at teh same time
            self.stack = []
        # Tells us when to go for kickoff
        # self.kickoff_flag = packet.game_info.is_round_active and packet.game_info.is_kickoff_pause
        self.kickoff_flag = packet.match_info.match_phase == flat.MatchPhase.Kickoff
    ...
    def get_output(self, packet):
        ...
        # print("preprocess + ready done") # Works
...
class car_object:
    ...
    def update(self, packet: flat.GamePacket):
        ...
        self.airborne = not car.air_state == flat.AirState.OnGround
        self.supersonic = car.is_supersonic
...
class ball_object:
    ...
    def update(self, packet: flat.GamePacket):
        ...
        # self.latest_touched_time = ball.latest_touch.time_seconds #TODO: Implement a counter or smth, this is not in v5
...
class boost_object:
    ...
    def update(self, packet: flat.GamePacket):
        self.active = packet.boost_pads[self.index].is_active
...
class game_object:
    ...
    def update(self, packet: flat.GamePacket):
        game = packet.match_info
        self.time = game.seconds_elapsed
        self.time_remaining = game.game_time_remaining
        self.overtime = game.is_overtime
        self.round_active = game.match_phase == flat.MatchPhase.Active
        self.kickoff = game.match_phase == flat.MatchPhase.Kickoff
        self.match_ended = game.match_phase == flat.MatchPhase.Ended
...
class Vector3:
    ...
    def __init__(self, *args):
        ...
        elif isinstance(args[0], flat.Vector3):
            self.data = [args[0].x, args[0].y, args[0].z]
        elif isinstance(args[0], flat.Rotator):
